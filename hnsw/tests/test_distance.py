...
def test_simple_2D():
    ...
    expected = np.array([0, 1, 3], dtype=np.float32)
...
@st.composite
def points_and_query(draw, min_n=1, max_n=50, min_d=1, max_d=200):
    d = draw(st.integers(min_value=min_d, max_value=max_d))
    n = draw(st.integers(min_value=min_n, max_value=max_n))
    points = draw(
        arrays(dtype=np.float32, shape=(n, d), elements=finite_floats)
    )
    query = draw(arrays(dtype=np.float32, shape=(d,), elements=finite_floats))
    return points, query
...
class TestL2DistProperties:
    ...
    def test_matches_oracle(self, data):
        ...
        result = l2_dist(points=points, query=query)
        expected = np.linalg.norm(points - query, axis=1)
    ...
    def test_output_shape_and_dtype(self, data):
        ...
        assert result.shape == (points.shape[0],)
